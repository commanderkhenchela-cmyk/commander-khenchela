...
| `delivery_drivers`, `deliveries` | V1 التوصيل يدوي بالكامل عبر Admin. تُضاف عند بناء تطبيق السائق، بربطها بـ `orders` دون تعديل الجداول الحالية |
| `payments` | V1 هو COD فقط. يكفي حقل `payment_status` داخل `orders`. الجدول الكامل يُضاف عند دمج بوابة دفع إلكتروني حقيقية |
| `reviews` | ليست جزءًا من حلقة الطلب الأساسية، تُضاف لاحقًا بدون أي تأثير على الموجود |
